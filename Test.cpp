...
void runLocate(unique_ptr<StringDictionary> &dict, char* in)
{
	cerr << "Running locate tests" << endl;
	ifstream inStrings(in);
	vector<string> strings;

	cerr << "Dictionary of size " << dict->numElements() << endl;
	cerr << "Reading pattern strings" << endl;
	string cur;
	while(getline(inStrings, cur, '\0')) {
		strings.push_back(cur);
		if(strings.size() % (dict->numElements()/10) == 0){
			cerr << strings.size() << endl;
		}
	}
	cerr << endl << "Finished reading. Total strings: " << strings.size() << endl;

	inStrings.close();

	uint64_t patterns = strings.size();
	double t0, t1, total=0;

	cerr << "Run start" << endl;

	for (uint_fast32_t i=1; i<=RUNS; i++)
	{
		t0 = getTime ();

		for (uint64_t j=0; j<patterns; j++){
			if(j%(patterns/10) == 0) cerr << "!";
			uint64_t located=dict->locate((uchar*)strings[j].data(), strings[j].size());
			if(located != j+1){
				cerr << "ERROR [str, located, j] = " << strings[j] << ' ' << located << ' ' << j+1 << endl;
			}
		} cerr << endl;
	...
	//for (uint64_t i=0; i<patterns; i++) delete [] strings[i];	
}

void runExtract(unique_ptr<StringDictionary> &dict, char* in)
{
	ifstream inIds(in);
	vector<uint> ids;
	char line[256];
...
void runLocatePrefix(unique_ptr<StringDictionary> &dict, char* in)
{
	ifstream inStrings(in);
...
void runExtractPrefix(unique_ptr<StringDictionary> &dict, char* in)
{
	ifstream inStrings(in);
...
void runLocateSubstring(unique_ptr<StringDictionary> &dict, char* in)
{
	ifstream inStrings(in);
...
void runExtractSubstring(unique_ptr<StringDictionary> &dict, char* in)
{
	ifstream inStrings(in);
...
void generate(unique_ptr<StringDictionary> &dict, uint patterns, char* out)
{
	srand (time(NULL));
...
void generateP(unique_ptr<StringDictionary> &dict, uint mean, char* out)
{
	srand (time(NULL));
	uint num = dict->numElements();
...
void generateS(unique_ptr<StringDictionary> &dict, uint mean, char* out)
{
	srand (time(NULL));
	uint num = dict->numElements();
...
main(int argc, char* argv[])
{
	if (argc >= 5)
	{
		...
		if (in.good())
		{
			unique_ptr<StringDictionary> dict = StringDictionary::load(in, HASHRP);
			in.close();
