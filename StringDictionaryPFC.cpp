...
StringDictionaryPFC::StringDictionaryPFC(IteratorDictString *it, uint_fast32_t bucketsize)
{
	this->type = PFC;
	this->elements = 0;
	this->maxlength = 0;
...
uint64_t
StringDictionaryPFC::locate(uchar *str, uint_fast32_t strLen)
{
	uint id = NORESULT;
...
uchar *
StringDictionaryPFC::extract(uint64_t id, uint_fast32_t *strLen)
{
	if ((id > 0) && (id <= elements))
	{
		...
		if (pos > 0) for (uint_fast32_t i=1; i<=pos; i++)
		{
...
IteratorDictString*
StringDictionaryPFC::extractSubstr(uchar *str, uint_fast32_t strLen)
{
...
uchar *
StringDictionaryPFC::extractRank(uint64_t rank, uint_fast32_t *strLen)
{
