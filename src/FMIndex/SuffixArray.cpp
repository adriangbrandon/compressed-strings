...
long *SuffixArray::sort(unsigned char *text, ulong length) {
    ...
    if (!p || !x) fprintf(stderr, "malloc failed\n");
...
#elif COMPACT == 2
        long *pi;
        for (long i = 0; i <= UCHAR_MAX; ++i) q[i] = 0;
        for (pi = x; pi < x + length; ++pi) q[*pi] = 1;
        for (long i = k = 0; i <= UCHAR_MAX; ++i)
            if (q[i]) q[i] = k++;
        for (pi = x; pi < x + length; ++pi) *pi = q[*pi] + 1;
        l = 1;
        ++k;
#endif
#endif
...
void SuffixArray::bucketsort(long *x, long *p, long n, long k) {
    ...
    for (pi = p; pi < p + k; ++pi) *pi = -1; /* mark linked lists empty.*/
    for (i = 0; i <= n; ++i) {
...
long SuffixArray::transform(long *x, long *p, long n, long k, long l, long q) {
    ...
    if (d <= n) {               /* if bucketing possible, compact alphabet.*/
        for (pi = p; pi <= p + d; ++pi) *pi = 0; /* zero transformation table.*/
