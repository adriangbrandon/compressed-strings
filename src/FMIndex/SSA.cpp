...
SSA::SSA(uchar *text, uint64_t n, bool free_text, uint samplesuff) {
    ...
    for (size_t i = 0; i < 256; i++) alphabet[i] = false;
}

SSA::~SSA() {
    if (_seq != NULL && free_text) delete[] _seq;
    if (_bwt != NULL) delete[] _bwt;
    if (_ssb != NULL) delete _ssb;
    if (bwt != NULL) delete (Sequence *)bwt;
    if (_sa != NULL) delete[] _sa;
    if (_sbb != NULL) delete _sbb;
    if (suff_sample != NULL) delete[] suff_sample;
    if (sampled != NULL) delete sampled;
...
void SSA::print_stats() {
    ...
    if (samplesuff > 0) {
        cout << " suff sample : " << sizeof(uint) * (1 + n / samplesuff)
             << endl;
...
bool SSA::build_index() {
    ...
    for (uint i = 0; i < maxV + 1; i++) occ[i] = 0;

    for (uint64_t i = 0; i <= n; i++) occ[_bwt[i] + 1]++;

    for (uint i = 1; i <= maxV; i++) occ[i] += occ[i - 1];
...
void SSA::build_sa() {
    ...
    delete suffix;
    assert(_sa[0] == n);
    for (uint64_t i = 0; i < n; i++) assert(cmp(_sa[i], _sa[i + 1]) <= 0);
}
    ...
    if (sp <= ep) {
        ...
        *occs = new size_t[matches + 1];
        ...
        while (i <= ep) {
            ...
            if (c != 1)
                (*occs)[i - sp] = suff_sample[sampled->rank1(j) - 1];
            else
                (*occs)[i - sp] = rank_tmp - 1;
...
uchar *SSA::extract_id(uint64_t id, uint_fast32_t *strLen,
    ...
    *strLen = 0;
    ...
    pos++;
    for (uint j = 0; j < (*strLen); j++) res[j] = res[pos + j];
    res[(*strLen)] = '\0';
    return res;
}
...
int64_t SSA::cmp(uint64_t i, uint64_t j) {
    while (i < n && j < n) {
        if (_seq[i] != _seq[j]) return (int64_t)_seq[i] - _seq[j];
        i++;
        j++;
    }
    assert(i != j);
