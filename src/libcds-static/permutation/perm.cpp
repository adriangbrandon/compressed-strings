...
#include "libcds-static/permutation/perm.h"
...
perm createPerm(uint *elems, uint nelems, uint t, BitSequenceBuilder *bmb) {
    ...
    if (t == 1) {
        ...
        for (i = 0; i < uint_len(nelems, 1); i++) b[i] = 0;
        assert(b != NULL);
        baux = new uint[uint_len(nelems, 1)];
        for (i = 0; i < uint_len(nelems, 1); i++) baux[i] = 0;
        assert(baux != NULL);
        nbwdptrs = 0;
        for (i = 0; i < nelems; i++) {
            if (bitget(baux, i) == 0) {
...
uint inversePerm(const perm P, uint i) {
    ...
    if (P->t == 1) {
        ...
        if (elem != i) {
            ...
            while ((elem = get_field(P->elems, P->nbits, j)) != i) j = elem;
        }
    }
    return j;
}
