...
MapperCont::MapperCont(const Array& seq, const BitSequenceBuilder& bmb) {
    ...
    for (size_t i = 0; i < seq.getLength(); i++) bs.setBit(seq[i]);
    m = bmb.build(bs);
}

MapperCont::MapperCont(const uint* A, const size_t len,
                       const BitSequenceBuilder& bmb) {
    uint max_v = 0;
    for (uint i = 0; i < len; i++) max_v = max(max_v, A[i]);
    max_v++;

    BitString bs(max_v);
    for (size_t i = 0; i < len; i++) bs.setBit(A[i]);
    m = bmb.build(bs);
}

MapperCont::MapperCont() {}

MapperCont::~MapperCont() { delete m; }

uint MapperCont::map(uint s) const { return m->rank1(s); }

uint MapperCont::unmap(uint s) const { return m->select1(s); }
