...
BitSequenceRG::BitSequenceRG(const BitString &bs, uint _factor) {
    ...
    for (size_t i = 0; i < uint_len(_n, 1); i++) data[i] = bitarray[i];
    for (size_t i = uint_len(_n, 1); i < _n / W + 1; i++) data[i] = 0;
    // this->owner = true;
    this->n = _n;
    uint lgn = bits(n - 1);
    this->factor = _factor;
...
void BitSequenceRG::BuildRank() {
    ...
    for (uint i = 0; i < num_sblock + 5; i++) Rs[i] = 0;
    size_t j;
    Rs[0] = 0;
...
size_t BitSequenceRG::rank1(const size_t i1) const {
    ...
    uint resp = Rs[i / s];
    uint aux = (i / s) * factor;
    for (uint a = aux; a < i / W; a++) resp += popcount(data[a]);
...
size_t BitSequenceRG::SpaceRequirementInBits() const {
    return uint_len(n, 1) * sizeof(uint) * 8 + (n / s) * sizeof(uint) * 8 +
           sizeof(BitSequenceRG) * 8;
}
