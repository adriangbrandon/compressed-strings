...
#include "libcds-static/bitsequence/sdarraySadakane.h"
...
int selectd2_construct(selectd2 *select, const int n,
...
int selectd2_select2(const selectd2 *select, const int i1, const int f, int *st,
                     int *en) {
    int i = i1;
    int p, r, p2;
    int il;
...
int selects3_construct(selects3 *select, const int n, const uint *buf) {
    ...
    select->hi = buf2;
    select->low = low;
    select->size = sizeof(unsigned char) * ((2 * m + 8 - 1) / 8 + 1) +
                   sizeof(uint) * ((d * m + PBS - 1) / PBS + 1);
