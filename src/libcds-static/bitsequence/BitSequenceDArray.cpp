...
BitSequenceDArray::BitSequenceDArray(const BitString &bs) {
    ...
    ones = 0;
    for (uint i = 0; i < uint_len(bs.getLength(), 1) + 1; i++) tmp_seq[i] = 0;
    for (uint i = 0; i < bs.getLength(); i++)
        if (bs[i]) {
            __setbit(tmp_seq, i, 1);
            ones++;
        }
    if (ones) build(tmp_seq, bs.getLength());
}

BitSequenceDArray::BitSequenceDArray(uint *buf, size_t n) {
    uint *tmp_seq = new uint[uint_len(n, 1) + 1];
    ones = 0;
    for (uint i = 0; i < uint_len(n, 1) + 1; i++) tmp_seq[i] = 0;
    for (uint i = 0; i < n; i++)
        if (bitget(buf, i)) {
            __setbit(tmp_seq, i, 1);
            ones++;
        }
    if (ones) build(tmp_seq, n);
    //	delete buf;
}
...
void BitSequenceDArray::build(uint *buf, size_t n) {
    ...
    m2 = 0;
    for (i = 0; i < (int)n; i++) m2 += __getbit(buf, i);
    ...
    for (r = 0; r < 2; r++) {
            ...
            if (p2 - pp >= (int)LL) {
                ...
                if (r == 1) {
                    for (is = 0; is < (int)(L / LLL); is++) {
                        if (il * (int)L + is * (int)LLL >= (int)m) break;
        ...
        if (r == 0) {
            ...
            s_ss = ms * (L / LLL) + 1;
            sl = new uint[s_sl];
            for (uint i_sl = 0; i_sl < s_sl; i_sl++) sl[i_sl] = 0;
            ss = new uint[s_ss];
            for (uint i_ss = 0; i_ss < s_ss; i_ss++) ss[i_ss] = 0;
        }
    }
    delete[] s;
    ...
    for (i = 0; i < (int)n; i += RR) {
        ...
        m2 = 0;
        for (j = 0; j < (int)RR; j++) {
            if (j % RRR == 0) rs[(i + j) / RRR] = m2;
            if (i + j < (int)n && __getbit(buf, i + j) == 1) m2++;
        }
        r += m2;
    }
}
