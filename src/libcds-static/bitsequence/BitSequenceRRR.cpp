...
#include "libcds-static/bitsequence/BitSequenceRRR.h"
...
void BitSequenceRRR::build(const uint *bitseq, size_t len, uint sample_rate) {
    ...
    for (uint i = 0; i < uint_len(C_len, C_field_bits); i++) C[i] = 0;
    O_bits_len = 0;
    for (uint i = 0; i < C_len; i++) {
        uint value = popcount(
            get_var_field(bitseq, i * BLOCK_SIZE,
                          min((uint)len - 1, (i + 1) * BLOCK_SIZE - 1)));
    ...
    O = new uint[O_len];
    for (uint i = 0; i < O_len; i++) O[i] = 0;
    uint O_pos = 0;
    for (uint i = 0; i < C_len; i++) {
        uint value =
            (ushort)get_var_field(bitseq, i * BLOCK_SIZE,
                                  min((uint)len - 1, (i + 1) * BLOCK_SIZE - 1));
        set_var_field(
            O, O_pos,
            O_pos + E->get_log2binomial(BLOCK_SIZE, popcount(value)) - 1,
            E->compute_offset((ushort)value));
...
void BitSequenceRRR::create_sampling(uint sample_rate) {
    ...
    C_sampling =
        new uint[max((uint)1, uint_len(C_sampling_len, C_sampling_field_bits))];
    for (uint i = 0;
         i < max((uint)1, uint_len(C_sampling_len, C_sampling_field_bits)); i++)
        C_sampling[i] = 0;
    uint sum = 0;
    for (uint i = 0; i < C_len; i++) {
        if (i % sample_rate == 0)
    ...
    // Sampling for O (table S) (Code separated from previous construction for
    // readability)
...
bool BitSequenceRRR::access(size_t i) const {
    ...
    return ((1 << (i % BLOCK_SIZE)) &
            E->short_bitmap(
                c, get_var_field(
                       O, pos_O,
                       pos_O + E->get_log2binomial(BLOCK_SIZE, c) - 1))) != 0;
}
...
size_t BitSequenceRRR::rank1(size_t i) const {
    ...
    uint sum =
        get_field(C_sampling, C_sampling_field_bits, nearest_sampled_value);
    ...
    while (k < (uint)max(0, (int)pos - 1)) {
        ...
        sum += ((*a) & mask) + (*a) / 16;
        pos_O += E->get_log2binomial(BLOCK_SIZE, ((*a) & mask)) +
                 E->get_log2binomial(BLOCK_SIZE, ((*a) / 16));
        a++;
        k += 2;
    }
    if (k < pos) {
    ...
    sum += popcount(
        ((2 << (i % BLOCK_SIZE)) - 1) &
        E->short_bitmap(
            c, get_var_field(O, pos_O,
                             pos_O + E->get_log2binomial(BLOCK_SIZE, c) - 1)));
    return sum;
}
...
size_t BitSequenceRRR::select0(size_t i) const {
    ...
    while (start < end - 1) {
        med = (start + end) / 2;
        acc = med * sample_rate * BLOCK_SIZE -
              get_field(C_sampling, C_sampling_field_bits, med);
        if (acc < i) {
            if (med == start) break;
            start = med;
        } else {
    ...
    while (start < C_len - 1 &&
           acc + sample_rate * BLOCK_SIZE ==
               get_field(C_sampling, C_sampling_field_bits, start + 1)) {
        start++;
        acc += sample_rate * BLOCK_SIZE;
    }
    ...
    while (acc < i) {
        ...
        size_t block =
            E->short_bitmap(s, get_var_field(O, pos_O, new_posO - 1));
        pos_O = new_posO;
        new_posO = 0;
...
size_t BitSequenceRRR::select1(size_t i) const {
    ...
    acc = get_field(C_sampling, C_sampling_field_bits, start);
    while (start < C_len - 1 &&
           acc == get_field(C_sampling, C_sampling_field_bits, start + 1))
        start++;
    pos = (start)*sample_rate;
    size_t pos_O = get_field(O_pos, O_pos_field_bits, start);
    acc = get_field(C_sampling, C_sampling_field_bits, start);
