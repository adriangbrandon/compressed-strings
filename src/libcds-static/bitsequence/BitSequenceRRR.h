...
#include "libcds-static/bitsequence/TableOffsetRRR.h"
...
/** Implementation of Raman, Raman and Rao's [1] proposal for rank/select
 * capable data structures, it achieves space nH_0, O(sample_rate) time for rank
 * and O(log len) for select. The practial implementation is based on [2]
 *
 *  [1] R. Raman, V. Raman and S. Rao. Succinct indexable dictionaries with
 * applications to encoding $k$-ary trees and multisets. SODA02. [2] F. Claude
 * and G. Navarro. Practical Rank/Select over Arbitrary Sequences. SPIRE08.
 *
...
class BitSequenceRRR : public BitSequence {
   public:
     ...
     */
    BitSequenceRRR(uint *bitseq, size_t len,
                   uint sample_rate = DEFAULT_SAMPLING);
