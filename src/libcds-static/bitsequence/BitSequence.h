...
/** Base class for static bitsequences, contains many abstract functions, so
 * this can't be instantiated. It includes base implementations for rank0,
 * select0 and select1 based on rank0.
 *
...
class BitSequence {
   public:
    ...
    /** Return the first position starting at i moving to the left that contains
     * a 1. In case there are no more ones to the left in the bitsequence, the
     * function returns (size_t)-1.
     */
    virtual size_t selectPrev1(const size_t i) const;

    /** Return the first position starting at i moving to the left that contains
     * a 0. In case there are no more zeros to the left in the bitsequence, the
     * function returns (size_t)-1.
     */
...
#include "libcds-static/bitsequence/BitSequenceRG.h"
#include "libcds-static/bitsequence/BitSequenceRRR.h"
#include "libcds-static/bitsequence/BitSequenceSDArray.h"
