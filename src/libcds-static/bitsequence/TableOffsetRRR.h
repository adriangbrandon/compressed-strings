 ...
 *  O(sample_rate) time for rank and O(log len) for select. The practial
...
class table_offset {
   public:
    ...
    inline uint get_binomial(uint n, uint k) { return binomial[n][k]; }
    ...
    /** Returns u */
    inline uint get_u() { return u; }

    /** Computes the offset of the first u bits of a given bitstring */
    inline ushort compute_offset(ushort v) { return rev_offset[v]; }
