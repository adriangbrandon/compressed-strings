...
uint generaClase(ushort* bch, uint u, uint clase, uint puestos, uint pos_ini,
                 uint generado);
uint offset_func(uint u, uint busca);
uint offsetRecursivo(uint u, uint busca, uint clase, uint puestos, uint pos_ini,
                     uint generado);
uint __indiceFunc;
uint __indAcumulado;
ushort* __Lis;
