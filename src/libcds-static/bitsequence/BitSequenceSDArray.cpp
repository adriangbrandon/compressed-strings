...
BitSequenceSDArray::BitSequenceSDArray(const BitString& bs) {
    ...
    if (ones) selects3_construct(&sd, bs.getLength(), tmp_seq);
...
BitSequenceSDArray::BitSequenceSDArray(uint* buff, size_t len) {
    ...
    ones = 0;
    for (uint i = 0; i < uint_len(len, 1) + 1; i++) tmp_seq[i] = 0;
    for (uint i = 0; i < len; i++)
        if (bitget(buff, i)) {
            __setbit(tmp_seq, i, 1);
            ones++;
        }
    if (ones) selects3_construct(&sd, len, tmp_seq);
    this->length = len;
    delete[] tmp_seq;
}

BitSequenceSDArray::BitSequenceSDArray() { make___selecttbl(); }

BitSequenceSDArray::~BitSequenceSDArray() {
    if (ones) selects3_free(&sd);
}
...
size_t BitSequenceSDArray::getSize() const {
    return sizeof(BitSequenceSDArray) +
