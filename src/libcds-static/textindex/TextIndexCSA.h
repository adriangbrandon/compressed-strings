...
#include "libcds-static/textindex/TextIndex.h"
#include "libcds-static/textindex/comparray4.h"
#include "libcds-static/textindex/interface.h"
...
class TextIndexCSA : public TextIndex {
   ...
   public:
     ...
     * 												free_text: The text will be freed immediately
     *after using it. samplerate=<number>: samplerate will be interval between
     *two indices of the suffix array stored explicitly. That is,
     * 												                     SA[i*samplerate] is stored
     *explicitly. The default value is 16. samplepsi=<number>: samplepsi will be
     *interval between two indices of the psi function stored explicitly. That
     *is, Psi[i*samplepsi] is stored explicitly. The default value is 128.
     **/
    ...
    virtual size_t locate(uchar *pattern, ulong length, ulong **occ,
                          ulong *numocc) const;
    ...
    virtual size_t extract(ulong from, ulong to, uchar **snippet,
                           ulong *snippet_length) const;
    ...
    virtual size_t display(uchar *pattern, ulong length, ulong numc,
                           ulong *numocc, uchar **snippet_text,
                           ulong **snippet_lengths) const;
