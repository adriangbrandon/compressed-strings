...
#include "libcds-static/textindex/mmap.h"
...
inline int getbitD(unsigned short *B, int i) {
    int j, l, x;
    i--;
    j = i >> 4;
    l = i & (DD - 1);
...
int initranktables(void) {
    ...
    error
#endif
        for (i = 0; i < TBLSIZE; i++) { /* DD==16 �ȊO�ł͓����Ȃ� */
        B = i;
        r = 0;
        for (m = 0; m < DD; m++) {
            b = getbit(&B, m + 1);
...
#endif
    int
    getzerorun(unsigned short *B, int p) {
    int w, w2;
#if 0
		w = 0;
...
inline int psi_list(CSA *SA, int i) {
    int j, l, r, m;
#ifdef DEBUG
    if (i > SA->n || i < 1) {
...
void csa_init(CSA *SA) {
    for (int i = 0; i < SIGMA + 2; i++) SA->K[i] = 0;
...
CSA *csa_load(ifstream &fp) {
    ...
    for (int i = 0; i < (SIGMA + 2); i++) SA->K[i] = loadValue<int>(fp);
    for (int i = 0; i < (SIGMA + 1); i++) SA->C[i] = loadValue<int>(fp);
    for (int i = 0; i < (SIGMA + 1); i++) SA->C2[i] = loadValue<int>(fp);
#ifdef USE_MMAP
...
inline int csa_psi(CSA *SA, int i) {
    int j, k, b, d, x;
    int k2, p, n;
    int l;
    unsigned short *B;
