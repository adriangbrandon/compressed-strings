...
void mygettime(mytimestruct *t) { ftime(t); }
...
static void parse_parameters(char *options, int *num_parameters,
                             char ***parameters, char *delimiters) {
...
static void free_parameters(int num_parameters, char ***parameters) {
    int i = 0;
    for (i = 0; i < num_parameters; i++) free((*parameters)[i]);
    free((*parameters));
}
...
int build_index(uchar *text, ulong length, char *build_options, void **index) {
    ...
    if (build_options != NULL) {
        parse_parameters(build_options, &num_parameters, &parameters,
                         delimiters);
        for (j = 0; j < num_parameters; j++) {
            if ((strcmp(parameters[j], "samplerate") == 0) &&
                (j < num_parameters - 1)) {
                rankb_w = atoi(parameters[j + 1]);
                j++;
            } else if ((strcmp(parameters[j], "samplepsi") == 0) &&
                       (j < num_parameters - 1)) {
    ...
    /* ³Ì¶ñÍ 0..n-1 Å n ÔÚÉÍ 0 ªüéB
                   p[0] ÍK¸ n
       ÉÈéBp[1..n]É0..n-1ªüÁÄ¢éB*/
    for (i = 0; i <= n; ++i)
        p[i]++; /* p[1..n]É1..nªüÁÄ¢éBp[0]=n+1*/
...
int free_index(void *index) {
    CSA *SA = (CSA *)index;
#ifdef USE_MMAP
    if (SA->mapp != NULL) free(SA->mapp);
    if (SA->mapi != NULL) free(SA->mapi);
#else
    free(SA->B) free(SA->ISA);
    free(SA->R);
    free(SA->SA);
#endif
    free(SA);
...
int display(void *index, uchar *pattern, ulong length, ulong numc,
    ...
    *snippet_text =
        (uchar *)malloc((*numocc) * (length + 2 * numc) * sizeof(uchar));
    ...
    for (i = 0; i < (*numocc); i++) {
        ...
        if (x > numc)
            ...
            from = 0;
        to = ((int)(x + length + numc - 1) < (int)(SA->n - 1)
                  ? (x + length + numc - 1)
                  : (SA->n - 1));
        len = to - from + 1;
