...
wt_coder_huff::wt_coder_huff(const Array& a, Mapper* am) {
    ...
    for (uint i = 0; i < n; i++) symbs[i] = am->map(a[i]);
    hc = new Huffman(symbs, n);
    maxBuffer = hc->maxLength() / W + 1;
    delete[] symbs;
}

wt_coder_huff::wt_coder_huff(uint* symbs, size_t n, Mapper* am) {
    for (uint i = 0; i < n; i++) symbs[i] = am->map(symbs[i]);
    hc = new Huffman(symbs, n);
    maxBuffer = hc->maxLength() / W + 1;
    for (uint i = 0; i < n; i++) symbs[i] = am->unmap(symbs[i]);
}

wt_coder_huff::wt_coder_huff(uchar* symbs, size_t n, Mapper* am) {
    for (uint64_t i = 0; i < n; i++) symbs[i] = (uchar)am->map((uint)symbs[i]);
    hc = new Huffman(symbs, n);
    maxBuffer = hc->maxLength() / W + 1;
