...
SequenceBuilderGMR::SequenceBuilderGMR(BitSequenceBuilder* bsb,
                                       SequenceBuilder* sqb, uint chunk_len) {
    this->bsb = bsb;
    this->sqb = sqb;
...
Sequence* SequenceBuilderGMR::build(uint* sequence, size_t len) {
    ...
    if (chunk_len == 0) {
        uint s = 0;
        for (size_t i = 0; i < len; i++) s = max(s, sequence[i]);
        cl = 2 * s;
    }
    cl = max(cl, (uint)64);
...
Sequence* SequenceBuilderGMR::build(const Array& seq) {
    ...
    if (chunk_len == 0) {
        uint s = 0;
        for (size_t i = 0; i < seq.getLength(); i++) s = max(s, seq[i]);
        cl = 2 * s;
    }
    cl = max(cl, (uint)64);
