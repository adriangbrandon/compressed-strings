...
#include "libcds-static/sequence/SequenceGMR.h"
namespace cds_static {

using std::max;
SequenceGMR::SequenceGMR(uint *sequence, size_t n, uint chunk_length,
    ...
    for (uint i = n; i < length; i++) new_seq[i] = sigma;
    if (length != n) sigma++;
    cout << "sigma=" << sigma << endl;
...
SequenceGMR::SequenceGMR(const Array &sequence, uint chunk_length,
                         BitSequenceBuilder *bmb, SequenceBuilder *ssb)
    : Sequence(0) {
    ...
    sigma++;
    cout << "sigma=" << sigma << endl;
    for (uint i = sequence.getLength(); i < length; i++) new_seq[i] = sigma;
...
SequenceGMR::~SequenceGMR() {
    delete B;
    for (uint i = 0; i < length / chunk_length; i++) delete chunk[i];
    delete[] chunk;
}

void SequenceGMR::build(uint *sequence, BitSequenceBuilder *bmb,
                        SequenceBuilder *ssb) {
    ...
    uint *B_bitmap =
        new uint[(2 + length + (unsigned long long)num_chunks * sigma) / W + 1];
    assert(B_bitmap != NULL);
    for (uint i = 0;
         i < (2 + length + (unsigned long long)num_chunks * sigma) / W + 1; i++)
        B_bitmap[i] = 0;
    uint pos = 0;
    for (unsigned long long i = 0; i < (unsigned long long)num_chunks * sigma;
         i++) {
...
uint *SequenceGMR::get_ones(uint *sequence) {
    ...
    assert(ones != NULL);
    for (uint i = 0; i < (unsigned long long)(length / chunk_length) * sigma;
         i++)
        ones[i] = 0;
    for (uint i = 0; i < length; i++) {
        uint whichChunk =
            (uint)(((unsigned long long)sequence[i] * length + i) /
                   chunk_length);
        ones[whichChunk]++;
    }
    return ones;
...
void SequenceGMR::save(ofstream &fp) const {
    ...
    B->save(fp);
    for (uint i = 0; i < length / chunk_length; i++) chunk[i]->save(fp);
}
