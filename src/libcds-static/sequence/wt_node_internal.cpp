...
wt_node_internal::wt_node_internal(uint *symbols, size_t n, uint l, wt_coder *c,
                                   BitSequenceBuilder *bmb) {
    uint *ibitmap = new uint[n / W + 1];
    for (uint64_t i = 0; i < n / W + 1; i++) ibitmap[i] = 0;
    for (uint64_t i = 0; i < n; i++) {
        if (c->is_set(symbols[i], l)) bitset(ibitmap, i);
    }
    ...
    if (count_right > 0) {
        if (match_right /* && c->done(right[0],l+1)*/)
            ...
            right_child =
                new wt_node_internal(right, count_right, l + 1, c, bmb);
    } else {
        right_child = NULL;
    }
    delete[] right;
...
wt_node_internal::wt_node_internal(uchar *symbols, size_t n, uint l,
                                   wt_coder *c, BitSequenceBuilder *bmb,
                                   size_t left, uint *done) {
    uint *ibitmap = new uint[n / W + 1];
    for (uint64_t i = 0; i < n / W + 1; i++) ibitmap[i] = 0;
    for (uint64_t i = 0; i < n; i++)
        if (c->is_set((uint)symbols[i + left], l)) bitset(ibitmap, i);
    bitmap = bmb->build(ibitmap, n);
    delete[] ibitmap;
    ...
    for (uint64_t i = 0; i < n; i++) set_field(done, 1, i + left, 0);
    ...
    for (uint64_t i = 0; i < n;) {
        ...
        while (get_field(done, 1, i + left)) ++i;
    }

    bool match_left = true, match_right = true;
    for (size_t i = 1; i < count_left; i++)
        if (symbols[i + left] != symbols[i + left - 1]) match_left = false;
    for (size_t i = count_left + 1; i < n; i++)
        if (symbols[i + left] != symbols[i + left - 1]) match_right = false;

    if (count_left > 0) {
        if (match_left /* && c->done(left[0],l+1)*/)
            left_child = new wt_node_leaf((uint)symbols[left], count_left);
        else
            left_child = new wt_node_internal(symbols, count_left, l + 1, c,
                                              bmb, left, done);
    } else {
        left_child = NULL;
    }
    if (count_right > 0) {
        if (match_right /* && c->done(right[0],l+1)*/)
            ...
            right_child = new wt_node_internal(symbols, count_right, l + 1, c,
                                               bmb, left + count_left, done);
    } else {
        right_child = NULL;
    }
}
...
size_t wt_node_internal::rank(uint symbol, size_t pos, uint l,
                              wt_coder *c) const {
...
size_t wt_node_internal::select(uint symbol, size_t pos, uint l,
                                wt_coder *c) const {
    bool is_set = c->is_set(symbol, l);
    size_t ret = 0;
    if (!is_set) {
        if (left_child == NULL) return (size_t)(-1);
        size_t new_pos = left_child->select(symbol, pos, l + 1, c);
        if (new_pos + 1 == 0) return (uint)(-1);
        ret = bitmap->select0(new_pos) + 1;
    } else {
        if (right_child == NULL) return (size_t)(-1);
...
size_t wt_node_internal::getSize() const {
    ...
    if (left_child != NULL) s += left_child->getSize();
    if (right_child != NULL) s += right_child->getSize();
    return s;
}
