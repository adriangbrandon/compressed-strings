...
SequenceAlphPart::SequenceAlphPart(const Array& seq, uint _cut,
    ...
    for (uint i = 0; i <= sigma; i++) pairs[i] = pair<size_t, uint>(occ[i], i);
    ...
    for (uint i = 0; i < sigma + 1; i++) alphSortedByFreq[i] = 0;

    uint* groupForSymb = new uint[origsigma + 1];
    for (uint i = 0; i < origsigma + 1; i++) groupForSymb[i] = 0;
    ...
    for (uint i = 0; i <= maxLen; i++) lenLength[i] = 0;
    ...
    for (uint i = 0; i < n; i++) tmpSeq[i] = groupForSymb[seq[i]];
    ...
    for (uint i = 0; i < maxLen + 1; i++) lenLength[i] = 0;

    for (uint i = 0; i < n; i++) {
        if (groupForSymb[seq[i]] > cut) {
            seqs[groupForSymb[seq[i]] - cut - 1]
                [lenLength[groupForSymb[seq[i]]]++] =
    ...
    for (uint i = 0; maxLen > cut && i < maxLen - cut; i++) {
        indexesByLength[i] =
            seqsBuilder->build(seqs[i], lenLength[i + cut + 1]);
        delete[] seqs[i];
    }
...
SequenceAlphPart::SequenceAlphPart(uint* seq, size_t n, uint cut,
                                   SequenceBuilder* lenIndexBuilder,
                                   SequenceBuilder* seqsBuilder)
    : Sequence(n) {
    // Say we are using the builders
...
size_t SequenceAlphPart::select(uint c, size_t i) const {
    ...
    uint g = group(pos, cut);
    uint o = offset(pos, cut, g);
    return groupsIndex->select(g,
                               indexesByLength[g - cut - 1]->select(o, i) + 1);
}
...
SequenceAlphPart* SequenceAlphPart::load(ifstream& fp) {
    ...
    if (type != ALPHPART_HDR) return NULL;
    ...
    ret->indexesByLength =
        new Sequence*[(ret->maxLen <= ret->cut) ? 0 : (ret->maxLen - ret->cut)];
