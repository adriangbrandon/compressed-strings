...
#include "libcds-static/sequence/WaveletTreeNoptrs.h"
...
WaveletTreeNoptrs::WaveletTreeNoptrs(const Array &a, BitSequenceBuilder *bmb,
    ...
    for (size_t i = 0; i < n; i++) symbols[i] = am->map(a[i]);
    ...
    for (uint i = 0; i < n; i++) new_symb[i] = symbols[i];
    ...
    uint new_n = n + to_add;
    for (uint i = 1; i <= max_v; i++) occurrences[i] += occurrences[i - 1];
    uint *oc = new uint[(new_n + 1) / W + 1];
    for (uint i = 0; i < (new_n + 1) / W + 1; i++) oc[i] = 0;
    for (uint i = 0; i <= max_v; i++) bitset(oc, occurrences[i] - 1);
    bitset(oc, new_n);
    ...
    for (uint i = 0; i < height; i++) {
        ...
        for (uint j = 0; j < new_n / W + 1; j++) _bm[i][j] = 0;
    }
...
WaveletTreeNoptrs::WaveletTreeNoptrs(uint *symbols, size_t n,
                                     BitSequenceBuilder *bmb, Mapper *am,
                                     bool deleteSymbols)
    : Sequence(n) {
    bmb->use();
    this->n = n;
    this->am = am;
    am->use();
    for (uint i = 0; i < n; i++) symbols[i] = am->map(symbols[i]);
    ...
    if (!deleteSymbols)
        for (uint i = 0; i < n; i++) symbols[i] = am->unmap(symbols[i]);
...
WaveletTreeNoptrs::WaveletTreeNoptrs(uint *symbols, size_t n, uint width,
                                     BitSequenceBuilder *bmb, Mapper *am,
                                     bool deleteSymbols)
    : Sequence(n) {
    bmb->use();
    ...
    uint *occurrences = new uint[max_v + 1];
    for (uint i = 0; i <= max_v; i++) occurrences[i] = 0;
    for (uint i = 0; i < n; i++) occurrences[get_field(symbols, width, i)]++;
    uint to_add = 0;
    for (uint i = 0; i < max_v; i++)
        if (occurrences[i] == 0) to_add++;
    ...
    if (!deleteSymbols)
        for (uint i = 0; i < n; i++)
            set_field(symbols, width, i,
                      am->unmap(get_field(symbols, width, i)));
...
WaveletTreeNoptrs::~WaveletTreeNoptrs() {
    if (bitstring) {
        for (uint i = 0; i < height; i++)
            if (bitstring[i]) delete bitstring[i];
        delete[] bitstring;
    }
    if (occ) delete occ;
    if (am) am->unuse();
...
void WaveletTreeNoptrs::save(ofstream &fp) const {
    ...
    saveValue(fp, height);
    am->save(fp);
    for (uint i = 0; i < height; i++) bitstring[i]->save(fp);
    occ->save(fp);
}
...
WaveletTreeNoptrs *WaveletTreeNoptrs::load(ifstream &fp) {
    ...
    for (uint i = 0; i < ret->height; i++) ret->bitstring[i] = NULL;
...
inline uint get_start(uint symbol, uint mask) { return symbol & mask; }
...
uint WaveletTreeNoptrs::access(size_t pos) const {
    ...
    while (level < height) {
        ...
        if (bitstring[level]->access(pos)) {
            ret = set(ret, level);
            pos = bitstring[level]->rank1(pos - 1) -
                  bitstring[level]->rank1(start - 1);
            start = (bitstring[level]->rank1(end) -
                     bitstring[level]->rank1(start - 1));
            start = end - start + 1;
            pos += start;
        } else {
...
size_t WaveletTreeNoptrs::rank(uint symbol, size_t pos) const {
    ...
    while (level < height) {
        if (is_set(symbol, level)) {
            pos = bitstring[level]->rank1(pos) -
                  bitstring[level]->rank1(start - 1) - 1;
            count = pos + 1;
            start = (bitstring[level]->rank1(end) -
                     bitstring[level]->rank1(start - 1));
            start = end - start + 1;
...
size_t WaveletTreeNoptrs::select(uint symbol, size_t j) const {
    ...
    while (true) {
        ...
        if (is_set(symbol, level)) {
            ...
            uint ones_start = bitstring[level]->rank1(start - 1);
            pos =
                bitstring[level]->select0(start - ones_start + pos) - start + 1;
        }
        mask <<= 1;
        sum <<= 1;
        if (level == 0) break;
...
void WaveletTreeNoptrs::build_level(uint **bm, uint *symbols, uint level,
    ...
    for (size_t i = 0; i < length; i++)
        if (!is_set(symbols[i], level)) cleft++;
...
// symbols is an array of elements of "width" bits.
void WaveletTreeNoptrs::build_level(uint **bm, uint *symbols, unsigned width,
                                    uint level, uint length, uint offset) {
    if (level == height) {
        delete[] symbols;
        return;
    }
    uint cleft = 0;
    for (size_t i = 0; i < length; i++)
        if (!is_set(get_field(symbols, width, i), level)) cleft++;
    uint cright = length - cleft;
...
uint WaveletTreeNoptrs::max_value(uint *symbols, size_t n) {
    uint max_v = 0;
    for (size_t i = 0; i < n; i++) max_v = max(symbols[i], max_v);
    return max_v;
}
...
size_t WaveletTreeNoptrs::count(uint symbol) const {
    return occ->select1(am->map(symbol)) - occ->select1(am->map(symbol) - 1) +
           1;
}
