...
BitmapsSequence::BitmapsSequence(uint *seq, size_t n, Mapper *am,
...
BitmapsSequence::BitmapsSequence(const Array &a, Mapper *am,
                                 BitSequenceBuilder *bsb)
    : Sequence(0) {
    bsb->use();
    length = a.getLength();
    ...
    uint pp = 0;
    for (uint i = 0; i < sigma; i++) {
        for (uint j = 0; j < uint_len(length, 1); j++) bm[j] = 0;
        while (pp < occ[i]) {
            bitset(bm, pos[pp]);
            pp++;
        }
...
size_t BitmapsSequence::getSize() const {
    ...
    for (uint i = 0; i < sigma; i++) size += bitmaps[i]->getSize();
    return size;
}
...
void BitmapsSequence::save(ofstream &fp) const {
    ...
    saveValue(fp, length);
    saveValue(fp, sigma);
    for (uint i = 0; i < sigma; i++) bitmaps[i]->save(fp);
    am->save(fp);
}
