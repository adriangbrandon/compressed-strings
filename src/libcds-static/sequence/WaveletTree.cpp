...
#include "libcds-static/sequence/WaveletTree.h"

namespace cds_static {

WaveletTree::WaveletTree(const Array &a, wt_coder *coder,
...
WaveletTree::WaveletTree(uint *symbols, size_t n, wt_coder *c,
                         BitSequenceBuilder *bmb, Mapper *am, bool free)
    : Sequence(n) {
    bmb->use();
    this->n = n;
    for (size_t i = 0; i < n; i++) symbols[i] = am->map(symbols[i]);
    this->am = am;
    am->use();
    this->c = c;
    c->use();
    ...
    if (free) {
        ...
        for (size_t i = 0; i < n; i++) symbols[i] = am->unmap(symbols[i]);
    }
    bmb->unuse();
}

WaveletTree::WaveletTree(uchar *symbols, size_t n, wt_coder *c,
                         BitSequenceBuilder *bmb, Mapper *am, bool free)
    : Sequence(n) {
    bmb->use();
    this->n = n;
    ...
    for (uint i = 0; i < n / W + 1; i++) done[i] = 0;
...
size_t WaveletTree::getSize() const {
    return sizeof(WaveletTree) + sizeof(uint) + root->getSize() +
           am->getSize() + c->getSize();
}
