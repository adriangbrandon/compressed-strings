...
class SequenceAlphPart : public Sequence {
   public:
    SequenceAlphPart(uint* seq, size_t n, uint cut,
                     SequenceBuilder* lenIndexBuilder,
                     SequenceBuilder* seqsBuilder);
    SequenceAlphPart(const Array& values, uint cut,
                     SequenceBuilder* lenIndexBuilder,
                     SequenceBuilder* seqsBuilder);
   ...
   protected:
    ...
    // cut corresponds to the number of symbols that are not represented in
    // indexesByLength (the most frequent ones)
    uint cut;
    uint origsigma;
