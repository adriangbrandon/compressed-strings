...
/** sets bit p in e */
inline void bitset(uint *e, size_t p) { e[p / W] |= (1 << (p % W)); }

/** sets bit p in e (64 bit) */
inline void bitset(uint64_t *e, size_t p) { e[p / W64] |= (1ULL << (p % W64)); }

/** cleans bit p in e */
inline void bitclean(uint *e, size_t p) { e[p / W] &= ~(1 << (p % W)); }
...
inline uint uint_len(const uint e, const size_t n) {
    return ((unsigned long long)e * n + W - 1) /
           W;  //+((unsigned long long)e*n%W>0));
}
