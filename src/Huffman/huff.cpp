...
THuffx createHuff(uint64_t *freq, uint lim) {
    THuffx H;
    int i, j, d;
    Ttree *tree;
    uint ptr, last, fre;
    ...
    H.depth = d;
    for (d = H.depth; d > 0; d--)
        H.num_enc[d] = H.num_enc[d - 1] - H.num_enc[d];
    H.num_enc[0] = (lim == 0);
    uint *Htmp = new uint[H.depth + 1];
    for (uint i = 0; i < H.depth + 1; i++) Htmp[i] = 0;
    for (uint i = 0; i < H.depth + 1; i++) Htmp[i] = H.num_enc[i];
    delete[] H.num_enc;
    H.num_enc = Htmp;
    H.total = 0;
    for (i = 0; i <= (int)lim; i++)
    ...
    uint aux;
    for (i = 0; i < (int)(H.lim + 1); i++) H.symb[i] = 0;
    for (i = 0; i <= (int)H.max; i++) {
        aux = H.spos[i];
        if (aux != (uint)~0) H.symb[aux] = i;
    }
    uint dold, dact;
...
THuffx loadHuff(ifstream &f) {
    ...
    H.spos = new uint[H.max + 1];
    for (i = 0; i <= H.max; i++) H.spos[i] = (uint)~0;
    for (i = 0; i <= H.lim; i++) H.spos[H.symb[i]] = i;
