...
Huffman::Huffman(uint *symb, size_t n) {
    uint max_v = 0;
    for (size_t i = 0; i < n; i++) max_v = max(max_v, symb[i]);
    uint64_t *occ = new uint64_t[max_v + 1];
    for (size_t i = 0; i < max_v + 1; i++) occ[i] = 0;
    for (size_t i = 0; i < n; i++) occ[symb[i]]++;
...
Huffman::Huffman(Array &seq) {
    ...
    uint max_v = 255;
    uint64_t *occ = new uint64_t[max_v + 1];
    for (size_t i = 0; i < (uint)max_v + 1; i++) occ[i] = 0;
    for (size_t i = 0; i < seq.getLength(); i++) occ[seq[i]]++;
    huff_table = createHuff(occ, max_v);
    delete[] occ;
}

Huffman::Huffman() { tree = NULL; }
...
size_t Huffman::maxLength() { return huff_table.depth; }

size_t Huffman::getSize() { return sizeof(Huffman) + sizeHuff(huff_table); }
...
DecodingTree *Huffman::obtainSubtree(uint symbol, uint k) {
...
void Huffman::deleteNode(BinaryNode *node) {
