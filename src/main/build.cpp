...
void checkDict() {
    ...
    cerr << " *****************************************************************"
            "*************** "
         << endl;
    cerr << " *** Checks the given file because it does not represent any "
            "valid dictionary *** "
         << endl;
    cerr << " *****************************************************************"
            "*************** "
...
void useBuild() {
    ...
    cerr << " \t <overhead> : hash table overhead over the number of strings."
         << endl;
    cerr << " \t <in> : input file containing the set of '\\0'-delimited "
            "strings."
         << endl;
    ...
    cerr << " \t <compress> : tecnique used for internal string compression."
         << endl;
    cerr << " \t              't' for HuTucker; 'h' for Huffman; 'r' for "
            "RePair compression."
         << endl;
    cerr << " \t <bucketsize> : number of strings per bucket." << endl;
    cerr << " \t <in> : input file containing the set of '\\0'-delimited "
        ...
        switch (type) {
                ...
                case 1:
                {
                        ...
                        if (in.good())
                        {
                                ...
                                IteratorDictString *it = new
                IteratorDictStringPlain(str, lenStr); StringDictionary *dict =
                NULL; string filename = string(argv[5]);

                                if (argv[2][0] == 'h')
                                {
                                        // Huffman compression
                                        dict = new StringDictionaryHASHHF(it,
                lenStr, overhead); filename += string(".hashhf");
                                }
                                else
                                {
                                        // RePair compression
                                        dict = new StringDictionaryHASHRPF(it,
                lenStr, overhead); filename += string(".hashrpf");
                                }
                ...
                */
                ...
                NULL; string filename = string(argv[5]);
                                ...
                                if (argv[2][0] == 'h')
                                {
                                        // Huffman compression
                                        dict = new
                StringDictionaryHASHUFFDAC(it, lenStr, overhead); filename +=
                string(".hashuffdac");
                                }
                                else
                                {
                                        // RePair compression
                                        dict = new StringDictionaryHASHRPDAC(it,
                lenStr, overhead); filename += string(".hashrpdac");
                                }
            ...
            case 3: {
                ...
                if (in.good()) {
                    ...
                    uint_fast32_t bucketsize = atoi(argv[3]);

                    uchar *str = loadValue<uchar>(in, lenStr);
                    in.close();

                    IteratorDictString *it =
                        new IteratorDictStringPlain(str, lenStr);
            ...
            case 4: {
                ...
                if (in.good()) {
                    ...
                    switch (argv[2][0]) {
                            ...
                            case 't':
                            {
                                    ...
                                    dict = new StringDictionaryHTFC(it,
                            bucketsize); filename += string(".htfc"); break;
                            }
                            */
                ...
                case 5:
                {
                        ...
                        if (in.good())
                        {
                                ...
                                uchar *str = loadValue<uchar>(in, lenStr);
                                IteratorDictString *it = new
                IteratorDictStringPlain(str, lenStr); in.close();

                                StringDictionary *dict = new
                StringDictionaryRPDAC(it); string filename =
                string(argv[3])+string(".rpdac");
            ...
            case 6: {
                ...
                if (in.good()) {
                    ...
                    if (argv[2][0] == 'p') {
                        // Plain bitmaps
                        dict = new StringDictionaryFMINDEX(
                            it, false, atoi(argv[3]), BWTsampling);
                        delete it;

                        filename = string(argv[6]) + string(".") +
                                   string(argv[3]) + string(".rg.fmi");

                    } else if (argv[2][0] == 'c') {
                        // Compressed bitmaps
                        dict = new StringDictionaryFMINDEX(
                            it, true, atoi(argv[3]), BWTsampling);
                        delete it;

                        filename = string(argv[6]) + string(".") +
                                   string(argv[3]) + string(".rrr.fmi");
                    } else {
                        checkFile();
                        break;
                    }
                ...
                case 7:
                {
                        ...
                        if (in.good())
                        {
                                ...
                                uchar *str = loadValue<uchar>(in, lenStr);
                                IteratorDictString *it = new
                IteratorDictStringPlain(str, lenStr-1); in.close();

                                StringDictionary *dict = new
                StringDictionaryXBW(it); delete it;

                                string filename =
                string(argv[3])+string(".xbw"); ofstream
                out((char*)filename.c_str()); dict->save(out); out.close();
                                delete dict;
                        }
                        else checkFile();
