...
HuTucker::HuTucker(uint64_t* occs) {
    this->start = 0;
    this->end = 255;
    this->max_v = 256;
...
void HuTucker::combination() {
    ...
    while (start != end) {
        ...
        // Update the sequence
        for (int i = pcom; i < end; i++) seq[i] = seq[i + 1];
        end--;
    }
...
uint HuTucker::findCompatibleNode(int node) {
    ...
    if (node != end) {
        ...
        for (int right = node + 2;
             (right <= end) && (seq[right - 1]->type != 0); right++)
    ...
    if (minleft == node)
        ...
        if (minright == node)
            ...
            pcom = (seq[minleft]->weight <= seq[minright]->weight) ? minleft
                                                                   : minright;
    }
