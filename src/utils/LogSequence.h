...
class LogSequence {
   ...
   private:
    ...
    inline size_t numElementsFor(const size_t bitsField,
     ...
     */
    inline size_t get_field(const size_t *data, const size_t bitsField,
                            const size_t index) {
        ...
        if (j + bitsField <= WLS) {
            ...
            result = data[i] >> j;
            result = result | (data[i + 1] << ((WLS << 1) - j - bitsField)) >>
                                  (WLS - bitsField);
        }
        return result;
    }
     ...
     */
    inline void set_field(size_t *data, const size_t bitsField,
