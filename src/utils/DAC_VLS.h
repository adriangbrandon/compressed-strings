...
#include "libcds-utils/libcdsTrees.h"
...
class DAC_VLS {
   ...
   public:
    ...
    /*Return the length of the sequence that start at pos (pos start from 1 to
     * n) and in seq store the sequence*/
    virtual uint access(uint pos, uint **seq) const;

    /*Return the value stored in position pos in the level l. It also store in
     * pos the position of the next value in the next level (if not exist next
     * value return (uint)-1 in pos*/
