...
void DecodingTable::setDecodingTable(uint k, DecodeableSubstr *substrs) {
    assert(k <= MAXK);
        ...
        if (substrs[i].dbits > 0) {
            ...
            if (substrs[i].length > 0) {
                ...
                if (!substrs[it->second].encoded) {
                    ...
                    stream[bytesStream] = encodeInfo(substrs[i].length,
                                                     substrs[it->second].dbits);
                    bytesStream++;
                    ...
                    table[i] = substrs[it->second].position;
                ...
                stream[bytesStream] =
                    encodeInfo(substrs[i].length, substrs[i].dbits);
                bytesStream++;

                bytesStream +=
                    VByte::encode(substrs[i].ptr, stream + bytesStream);
            }
        } else
            table[i] = 0;
    }
...
uint DecodingTable::setDecodingSubtree(DecodingTree *tree) {
    subtrees[nodes] = tree;
    return nodes++;
}

bool DecodingTable::getSubstring(ChunkScan *c) {
    ...
    if (x.length != 0) {
        ...
        for (uint i = 0; i < x.length; i++) {
            c->str[c->strLen + i] = stream[position + i];
        }
...
bool DecodingTable::processChunk(ChunkScan *c) {
    while (c->c_valid < k) {
        if (c->b_remain == 0) {
            if (c->c_valid < k)
...
size_t DecodingTable::getSize() {
    uint treesize = 0;
    for (uint i = 0; i < nodes; i++) treesize += subtrees[i]->getSize();

    return pow(2, k) * sizeof(uint) + bytesStream * sizeof(uchar) +
...
DecodingTable *DecodingTable::load(ifstream &in) {
    ...
    for (uint i = 0; i < table->nodes; i++)
        table->subtrees[i] = DecodingTree::load(in);
...
inline uchar DecodingTable::encodeInfo(uint length, uint bits) {
    return (uchar)((length << 4) | (bits - 1));
}

inline void DecodingTable::decodeInfo(uchar code, uint *length, uint *bits) {
    *length = ((code & 240) >> 4);
    *bits = ((code & 15) + 1);
}
