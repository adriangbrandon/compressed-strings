...
uint_fast32_t StatCoder::encodeSymbol(uchar symbol, uchar *text,
    ...
    while ((bits - processed) >= (8 - (*offset))) {
        uchar code =
            ((codeword << (W - bits + processed)) >> (W - 8 + (*offset)));
        text[bytes] |= code;
...
uchar *StatCoder::encodeString(uchar *str, uint_fast32_t strLen,
                               uint_fast32_t *encLen, uint_fast32_t *offset) {
...
uint StatCoder::decodeString(ChunkScan *c) {
    c->extracted = 0;
