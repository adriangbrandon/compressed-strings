 ...
 * used in the text. In these cases, the corresponding entries store the value
 *0.
 *
...
struct ChunkScan {
    uint_fast32_t c_chunk;  // Bytes to be currently processed
    ushort c_valid;         // Number of 'valid' bits in the current chunk

    uchar *b_ptr;  // Pointer to the current byte in the dictionary chunk
    uint_fast32_t
        b_remain;  // Number of remaining bytes in the dictionary chunk

    uchar *str;               // Extracted string
    uint_fast32_t strLen;     // String length
    uint_fast32_t advanced;   // Number of bytes extracted in advance
    uint_fast32_t extracted;  // Tells the number of extracted chars
};
...
class DecodingTable {
   ...
   public:
    ...
    DecodingTable(uint sigma) {
        this->nodes = 0;
        this->subtrees = new DecodingTree *[sigma];
    };
