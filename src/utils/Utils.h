...
#define GET_TIME_DIVIDER \
    ((double)1.0)  // getTime deals with "seconds" as time unit.
...
static const uint32_t HASHBRP =
    2;  // HashB-RePair dictionary (used for loading purposes)
static const uint32_t HASHBBRP =
    3;  // HashBB-RePair dictionary (used for loading purposes)

static const uint32_t HASHUFF =
    1;  // Hash-Huffman dictionary (used for loading purposes)
static const uint32_t HASHBHUFF =
    2;  // HashB-Huffman dictionary (used for loading purposes)
static const uint32_t HASHBBHUFF =
    3;  // HashBB-Huffman dictionary (used for loading purposes)

/* Front-Coding based dictionaries */
static const uint32_t PFC = 211;  // Plain Front-Coding dictionary
static const uint32_t RPFC =
    214;  // Plain Front-Coding dictionary (with RePair for suffixes)
static const uint32_t HTFC = 221;  // HuTucker Front-Coding dictionary
static const uint32_t HHTFC =
    222;  // HuTucker Front-Coding dictionary (with Huffman for suffixes)
static const uint32_t RPHTFC =
    223;  // HuTucker Front-Coding dictionary (with RePair for suffixes)

/* RePair+DAC dictionary */
static const uint32_t RPDAC = 3;  // RePair+DAC dictionary

/* FM-Index based dictionaries */
static const uint32_t FMINDEX =
    4;  // FM-Index dictionary (with plain: RG and compressed: RRR variants)

/* FM-Index based dictionaries */
static const uint32_t DXBW =
    5;  // XBW dictionary (with plain: RG and compressed: RRR variants)

inline uint encodeVB2(uint c, uchar *r) {
    unsigned int i = 0;
...
inline uint decodeVB2(uint *c, uchar *r) {
    *c = 0;
    int i = 0;
    int shift = 0;
...
inline ushort mask(uint k) { return (65535u >> (16u - k)); }

inline size_t Reallocate(uchar **array, size_t len) {
    size_t llen = len * 2;
    ...
    for (size_t i = len; i < llen; i++) xarr[i] = 0;
...
inline size_t Reallocate(int **array, size_t len) {
    size_t llen = len * 2;
    ...
    // This initialization is due to valgrind warnings
    for (uint64_t i = len; i < llen; i++) xarr[i] = 0;
...
inline int longestCommonPrefix(const uchar *str1, const uchar *str2,
                               uint_fast32_t length, uint_fast32_t *lcp) {
    uint ptr = 0;
