...
DAC_VLS::DAC_VLS(int *list, uint l_Length, uint log_r, uint max_seq_length) {
    ...
    for (uint i = 0; i < nLevels; i++) levelSizeAux[i] = 0;
    ...
    uint tamLevels = 0;
    for (uint i = 0; i < nLevels; i++) tamLevels += base_bits * levelSizeAux[i];
    tamCode = tamLevels;
    ...
    for (uint i = 0; i < (tamLevels / W + 1); i++) levels[i] = 0;
    ...
    for (uint i = 0; i < ((bits_BS_len) / W + 1); i++) bits_BS[i] = 0;
    ...
    for (uint i = 0; i < l_Length; i++) {
        for (uint j = 0; j < nLevels; j++) {
            if (list[i] >= 0) {
                ...
                contB[j]++;
                i++;
                if (j > 0) bitset(bits_BS, contB[j - 1] - 1);
            } else
                break;
        }
    }
...
uint DAC_VLS::getListLength() const { return listLength; }
