...
StringDictionaryPFC::StringDictionaryPFC(IteratorDictString *it,
    ...
    while (it->hasNext()) {
        ...
        if ((elements % bucketsize) == 0) {
            ...
            strncpy((char *)(textStrings + bytesStrings),
                    (char *)strCurrent + lcp, lenCurrent - lcp);
    ...
    if (cmp)
        ...
        if (idbucket != NORESULT) {
            ...
            if (scanneable > 1) {
                ...
                cmp =
                    longestCommonPrefix(decoded + sharedCurr, str + sharedCurr,
                    ...
                    for (uint_fast32_t i = 2; i < scanneable; i++) {
                        ...
                        if (sharedPrev == sharedCurr)
                            cmp = longestCommonPrefix(
                                decoded + sharedCurr, str + sharedCurr,
                                decLen - sharedCurr + 1, &sharedCurr);
...
uchar *StringDictionaryPFC::extract(uint64_t id, uint_fast32_t *strLen) {
    if ((id > 0) && (id <= elements)) {
...
IteratorDictID *StringDictionaryPFC::locatePrefix(uchar *str, uint strLen) {
    ...
    if (leftBucket > NORESULT) {
        ...
        if (leftBucket == rightBucket) {
            ...
            leftID =
                searchPrefix(&ptr, scanneable, decoded, &decLen, str, strLen);
            ...
            if (leftID == NORESULT)
                ...
                rightID = leftID +
                          searchDistinctPrefix(ptr, scanneable - leftID,
                                               decoded, &decLen, str, strLen) -
                          1;
            ...
            // All prefixes exceed (possibly) a single bucket
            {
                ...
                leftID = searchPrefix(&ptr, scanneable, decoded, &decLen, str,
                                      strLen);
...

            {
                ...
                rightID = searchDistinctPrefix(ptr, scanneable - 1, decoded,
                                               &decLen, str, strLen);
                rightID += (rightBucket - 1) * bucketsize;
            }
        }
...
IteratorDictString *StringDictionaryPFC::extractPrefix(uchar *str,
    ...
    if (it->getLeftLimit() != NORESULT) {
        ...
        return new IteratorDictStringPFC(textStrings + ptrS, leftpos,
                                         bucketsize, right - left + 1,
                                         maxlength);
    } else
        return NULL;
}
...
uchar *StringDictionaryPFC::extractRank(uint64_t rank, uint_fast32_t *strLen) {
...
IteratorDictString *StringDictionaryPFC::extractTable() {
    ...
    return new IteratorDictStringPFC(textStrings + ptrS, 0, bucketsize,
                                     elements, maxlength);
}
...
unique_ptr<StringDictionary> StringDictionaryPFC::load(ifstream &in) {
    size_t type = loadValue<uint32_t>(in);
    if (type != PFC) return NULL;

    auto dict = make_unique<StringDictionaryPFC>();
...
StringDictionaryPFC::~StringDictionaryPFC() { delete[] textStrings; }
