...
StringDictionaryRPHTFC::StringDictionaryRPHTFC(IteratorDictString *it,
                                               uint32_t bucketsize) {
    this->type = RPHTFC;
    ...
    cerr << "2) Obtaining the char frequencies and building the Hu-Tucker tree"
         << endl;
    ...
    uint64_t reservedInts = elements;
    int *rpdict = new int[reservedInts];

    // Initializing counters
    for (uint32_t i = 0; i < 256; i++) freqs[i] = 1;

    size_t pbeg = 0, pend = 0;
    uint64_t bucket = 1;
    uint64_t buckets = dict->buckets;
    uint64_t ptrpdict = 0;

    while (bucket <= buckets) {
        // Recollecting statistics for the headers and extracting them for
        ...
        // Stores the last position with 0 to avoid confusions with 0 values
        // encoding VBytes.
        uint64_t zero = pbeg - 1;

        // Extracting the internal strings for Re-Pair compression
        for (; pbeg < pend; pbeg++) {
            int c = (int)(dict->textStrings[pbeg]);

            if ((c != 0) || ((c == 0) && (pbeg == zero + 1)))
                rpdict[ptrpdict] = c;
            else {
                zero = pbeg;
    ...
    cerr << "3) Compressing the dictionary and building the decoding table"
         << endl;
    // 3) Compressing the dictionary and building the decoding table
    {
        ...
        for (bucket = 1; bucket <= buckets; bucket++) {
            ...
            do {
                ...
                builderHT->insertDecodeableSubstr(
                    symbol, &codeSubstr, &ptrSubstr, &textSubstr, &lenSubstr);
            ...
            // Updating the ptr value to the beginning of the corresponding
            // internal string
            ...
            if (textSubstr.size() > 0) {
                // #######################
                // It is necessary to read up to TABLEBITSO bits for indexing
                // the substring in the DecodingTable
                ...
                if (ptrSubstr > TABLEBITSO) {
                    ...
                    if ((bucket == buckets) && (elements % bucketsize == 0)) {
                        ...
                        // The first symbol encoding the internal strings is
                        // enough for padding because it uses, at least, 16 bits
                        uint remaining = TABLEBITSO - ptrSubstr;
                        uint codeword = intStrings[ptrB];

                        if (remaining < bitsrp) {
                            codeSubstr = (codeSubstr << remaining) |
                                         (codeword >> (bitsrp - remaining));
                        } else {
            ...
            for (; ptrB <= ptrE; ptrB++)
                bytesStrings += encodeSymbol(
                    intStrings[ptrB], &(textStrings[bytesStrings]), &offset);
...
uint64_t StringDictionaryRPHTFC::locate(uchar *str, uint_fast32_t strLen) {
    ...
    if (cmp)
        ...
        if (idbucket != NORESULT) {
            ...
            uint_fast32_t scanneable = bucketsize;
            ...
            if (scanneable > 1) {
                ...
                if (cmp != 0) {
                    for (uint_fast32_t i = 2; i < scanneable; i++) {
                        sharedPrev =
...
uchar *StringDictionaryRPHTFC::extract(uint64_t id, uint_fast32_t *strLen) {
    if ((id > 0) && (id <= elements)) {
        uint64_t idbucket = 1 + ((id - 1) / bucketsize);
        uint_fast32_t pos = ((id - 1) % bucketsize);

        ChunkScan c = decodeHeader(idbucket);

        if (pos > 0) {
            uint_fast32_t offset = 0;
            for (uint_fast32_t i = 1; i <= pos; i++)
                decodeString(c.str, &c.strLen, &c.b_ptr, &offset);
        }
...
IteratorDictID *StringDictionaryRPHTFC::locatePrefix(uchar *str, uint strLen) {
    // Encoding the string
    uint_fast32_t encLen, offset = 0;
    ...
    if (leftBucket > NORESULT) {
        ...
        if (leftBucket == rightBucket) {
...

            {
                ...
                offset = 0;
                rightID =
                    searchDistinctPrefix(&c, scanneable, str, strLen, &offset);
                rightID += (rightBucket - 1) * bucketsize;
            }
        }
...
IteratorDictID *StringDictionaryRPHTFC::locateSubstr(uchar * /* str */,
                                                     uint /* strLen */) {
    cerr << "This dictionary does not provide substring location" << endl;
    return NULL;
}

uint StringDictionaryRPHTFC::locateRank(uint rank) { return rank; }

IteratorDictString *StringDictionaryRPHTFC::extractPrefix(uchar *str,
                                                          uint strLen) {
    ...
    if (left != NORESULT) {
        ...
        return new IteratorDictStringRPHTFC(
            tableHT, codewordsHT, rp, bitsrp, textStrings, blStrings,
            leftbucket, leftpos, bucketsize, right - left + 1, maxlength,
            maxcomplength);
    } else
        return NULL;
}

IteratorDictString *StringDictionaryRPHTFC::extractSubstr(
    uchar * /* str */, uint_fast32_t /* strLen */) {
    cerr << "This dictionary does not provide substring extraction" << endl;
    return 0;
}

uchar *StringDictionaryRPHTFC::extractRank(uint64_t rank,
                                           uint_fast32_t *strLen) {
    return extract(rank, strLen);
}

IteratorDictString *StringDictionaryRPHTFC::extractTable() {
    return new IteratorDictStringRPHTFC(
        tableHT, codewordsHT, rp, bitsrp, textStrings, blStrings, 1, 0,
        bucketsize, elements, maxlength, maxcomplength);
}

size_t StringDictionaryRPHTFC::getSize() {
    return bytesStrings * sizeof(uchar) + blStrings->getSize() +
           256 * sizeof(Codeword) + tableHT->getSize() + rp->getSize() +
...
uchar *StringDictionaryRPHTFC::getHeader(size_t idbucket) {
...
ChunkScan StringDictionaryRPHTFC::decodeHeader(size_t idbucket) {
...
bool StringDictionaryRPHTFC::locateBucket(uchar *str, uint strLen,
                                          size_t *idbucket) {
...
void StringDictionaryRPHTFC::locateBoundaryBuckets(uchar *str, uint strLen,
...
uint StringDictionaryRPHTFC::searchPrefix(ChunkScan *c, uint scanneable,
...
uint_fast32_t StringDictionaryRPHTFC::searchDistinctPrefix(
    ChunkScan *c, uint_fast32_t scanneable, uchar * /* str */,
    uint_fast32_t strLen, uint_fast32_t *offset) {
    uint_fast32_t id = 1;
...
uint_fast32_t StringDictionaryRPHTFC::encodeSymbol(uint_fast32_t symbol,
                                                   uchar *text,
                                                   uint_fast32_t *offset) {
    uint processed = 0;
    uint bytes = 0;
    ...
    while ((bitsrp - processed) >= (8 - (*offset))) {
        uchar code =
            ((symbol << (W - bitsrp + processed)) >> (W - 8 + (*offset)));
        text[bytes] |= code;
...
inline uint_fast32_t StringDictionaryRPHTFC::decodeSymbol(
    uint_fast32_t *symbol, uchar *ptr, uint_fast32_t *offset) {
    *symbol = 0;
    ...
    while ((bitsrp - processed) >= (8 - (*offset))) {
        *symbol = (*symbol << (8 - (*offset))) |
                  (*(ptr + bytes) & mask(8 - (*offset)));
    ...
    if (bitsrp > processed) {
        *offset = bitsrp - processed;
        *symbol = (*symbol << (bitsrp - processed)) |
                  (*(ptr + bytes) >> (8 - *offset));
    }
...
inline uint StringDictionaryRPHTFC::decodeString(uchar *str,
                                                 uint_fast32_t *strLen,
                                                 uchar **ptr,
                                                 uint_fast32_t *offset) {
    ...
    while (read < 2) {
        *ptr += decodeSymbol(&rule, *ptr, offset);
        ...
        if (rule >= rp->terminals)
            read += rp->expandRule(rule - rp->terminals, vb + read);
        else {
            vb[read] = (uchar)rule;
            read++;
        }
