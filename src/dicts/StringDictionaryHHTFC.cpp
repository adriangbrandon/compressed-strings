...
StringDictionaryHHTFC::StringDictionaryHHTFC(IteratorDictString *it, uint bucketsize)
{
	...
	for (uint32_t i=0; i<256; i++) { freqsHT[i]=1; freqsHU[i]=1; }
	...
	// 3) Compressing the dictionary and building the decoding table
	{
		...
		size_t ptr = 0; uint_fast32_t offset = 0, bytes = 0;
...
uint64_t 
StringDictionaryHHTFC::locate(uchar *str, uint_fast32_t strLen)
{
...
uchar *
StringDictionaryHHTFC::extract(uint64_t id, uint_fast32_t *strLen)
{
	if ((id > 0) && (id <= elements))
	{
...
IteratorDictString*
StringDictionaryHHTFC::extractSubstr(uchar *str, uint_fast32_t strLen)
{
...
uchar *
StringDictionaryHHTFC::extractRank(uint64_t rank, uint_fast32_t *strLen)
{
...
StringDictionaryHHTFC::load(ifstream &in)
{
	...
	unique_ptr<StringDictionaryHHTFC> dict = make_unique<StringDictionaryHHTFC>();
