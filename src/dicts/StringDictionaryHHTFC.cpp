...
StringDictionaryHHTFC::StringDictionaryHHTFC(IteratorDictString *it,
                                             uint bucketsize) {
    this->type = HHTFC;
    ...
    // 2) Obtaining the char frequencies and the corresponding Hu-Tucker and
    // Huffman
    //    trees for the headers and the internal strings respectively
    uint64_t *freqsHT = new uint64_t[256];
    uint64_t *freqsHU = new uint64_t[256];
    ...
    while (bucket <= dict->buckets) {
        ...
        for (; dict->textStrings[pbeg] != 0; pbeg++)
            freqsHT[(int)(dict->textStrings[pbeg])]++;
        freqsHT[0]++;
        pbeg++;
    ...
    // 3) Compressing the dictionary and building the decoding table
    {
        ...
        size_t ptr = 0;
        uint_fast32_t offset = 0, bytes = 0;
        ...
        for (uint64_t current = 1; current <= elements; current++) {
            ...
            if (((current - 1) % bucketsize) == 0) {
                ...
                do {
                    ...
                    bytes +=
                        coderHT->encodeSymbol(symbol, &(tmp[bytes]), &offset);
                    ptr++;

                    builderHT->insertDecodeableSubstr(symbol, &codeSubstr,
                                                      &ptrSubstr, &textSubstr,
                                                      &lenSubstr);
                ...
                if (textSubstr.size() > 0) {
                    // #######################
                    // It is necessary to read up to TABLEBITSO bits for
                    // indexing the substring in the DecodingTable
                    if (offset > 0) {
                    ...
                    if (ptrSubstr > TABLEBITSO) {
                        ...
                        if (current == elements) {
                            ...
                            codeSubstr =
                                (codeSubstr << (TABLEBITSO - ptrSubstr));
                            ptrSubstr = TABLEBITSO;
                            break;
                        }

                        uint64_t read = 0;
                        uint64_t nextst = current + 1;

                        while (TABLEBITSO > ptrSubstr) {
                            {
                                // The VByte value is independently encoded to
                                // avoid confusions due to '\0' bytes
                                uint_fast32_t value = 0;
                                size_t xptr = VByte::decode(
                                    &value, &(dict->textStrings[ptr]));

                                for (uint i = 0; i < xptr; i++) {
                                    uint symbol = dict->textStrings[ptr + read];
                                    read++;
                                    uint bits = codewordsHU[(int)symbol].bits;
                                    uint codeword =
                                        codewordsHU[(int)symbol].codeword;

                                    if ((bits + ptrSubstr) <= TABLEBITSO) {
                                        codeSubstr =
                                            (codeSubstr << bits) | codeword;
                                        ptrSubstr += bits;
                                        ...
                                        codeSubstr =
                                            (codeSubstr << remaining) |
                                            (codeword >> (bits - remaining));
                                        ptrSubstr = TABLEBITSO;
                                ...
                                if ((bits + ptrSubstr) <= TABLEBITSO) {
                                    ...
                                    if (symbol == 0) {
                                        if ((nextst % bucketsize) == 0) {
                                            if (((ptrSubstr + (8 - offset)) >
                                                 TABLEBITSO)) {
                                                offset = offset % 8;

                                                // The padding bits are
                                                // enough...
                                                codeSubstr =
                                                    (codeSubstr << (TABLEBITSO -
                                                                    ptrSubstr));
                                                ptrSubstr = TABLEBITSO;
                                            } else {
                                                while (true) {
                                                    uint symbol =
                                                        dict->textStrings[ptr +
                                                                          read];
                                                    read++;
                                                    uint bits =
                                                        codewordsHT[(int)symbol]
                                                            .bits;
                                                    uint codeword =
                                                        codewordsHT[(int)symbol]
                                                            .codeword;
                    ...
                    builderHT->insertEndingSubstr(&codeSubstr, &ptrSubstr,
                                                  &textSubstr, &lenSubstr);
                }
                ...
                for (uint i = 0; i < xptr; i++) {
                    ...
                    bytesStrings += coderHU->encodeSymbol(
                        code, &(textStrings[bytesStrings]), &offset);
                    ptr++;

                    builderHU->insertDecodeableSubstr(
                        code, &codeSubstr, &ptrSubstr, &textSubstr, &lenSubstr);
                }
                ...
                do {
                    ...
                    uchar symbol = (dict->textStrings[ptr]);
                    bytesStrings += coderHU->encodeSymbol(
                        symbol, &(textStrings[bytesStrings]), &offset);
                    ptr++;

                    builderHU->insertDecodeableSubstr(symbol, &codeSubstr,
                                                      &ptrSubstr, &textSubstr,
                                                      &lenSubstr);
                } while (dict->textStrings[ptr - 1] != '\0');
                ...
                if ((current % bucketsize) == 0) {
                    ...
                    // Adding an ending decodeable string (if required)
                    if (textSubstr.size() > 0)
                        builderHU->insertEndingSubstr(&codeSubstr, &ptrSubstr,
                                                      &textSubstr, &lenSubstr);
        ...
        maxcomplength += 4;  // The value is increased because advanced readings
                             // in decoding...
    }

    delete dict;

    tableHT = builderHT->getTable();
    delete builderHT;
    tableHU = builderHU->getTable();
    delete builderHU;
}

uint64_t StringDictionaryHHTFC::locate(uchar *str, uint_fast32_t strLen) {
    uint64_t id = NORESULT;
    // Encoding the string
    uint_fast32_t encLen, offset;
    ...
    if (cmp)
        ...
        if (idbucket != NORESULT) {
            ...
            if (scanneable > 1) {
                uint_fast32_t sharedCurr = 0, sharedPrev = 0;
                int cmp = 0;

                // Processing the first internal string
                sharedPrev = coderHU->decodeString(&c);
                cmp = longestCommonPrefix(c.str + sharedCurr, str + sharedCurr,
                                          c.strLen - sharedCurr, &sharedCurr);
                ...
                if (cmp != 0) {
                    for (uint i = 2; i < scanneable; i++) {
                        ...
                        cmp = longestCommonPrefix(
                            c.str + sharedCurr, str + sharedCurr,
                            c.strLen - sharedCurr, &sharedCurr);
...
uchar *StringDictionaryHHTFC::extract(uint64_t id, uint_fast32_t *strLen) {
    if ((id > 0) && (id <= elements)) {
...
IteratorDictID *StringDictionaryHHTFC::locatePrefix(uchar *str, uint strLen) {
    // Encoding the string
    uint_fast32_t encLen, offset;
    ...
    if (leftBucket > NORESULT) {
        ...
        if (leftBucket == rightBucket) {
            ...
            if (leftID == NORESULT)
                ...
                rightID = leftID +
                          searchDistinctPrefix(&c, scanneable - leftID + 1, str,
                                               strLen) -
                          1;
...
IteratorDictID *StringDictionaryHHTFC::locateSubstr(uchar *str, uint strLen) {
    cerr << "This dictionary does not provide substring location" << endl;
    return NULL;
}

uint StringDictionaryHHTFC::locateRank(uint rank) { return rank; }

IteratorDictString *StringDictionaryHHTFC::extractPrefix(uchar *str,
                                                         uint strLen) {
    ...
    if (left != NORESULT) {
        ...
        return new IteratorDictStringHHTFC(
            tableHT, tableHU, codewordsHT, textStrings, blStrings, leftbucket,
            leftpos, bucketsize, right - left + 1, maxlength, maxcomplength);
    } else
        return NULL;
}

IteratorDictString *StringDictionaryHHTFC::extractSubstr(uchar *str,
                                                         uint_fast32_t strLen) {
    cerr << "This dictionary does not provide substring extraction" << endl;
    return 0;
}

uchar *StringDictionaryHHTFC::extractRank(uint64_t rank,
                                          uint_fast32_t *strLen) {
    return extract(rank, strLen);
}

IteratorDictString *StringDictionaryHHTFC::extractTable() {
    return new IteratorDictStringHHTFC(tableHT, tableHU, codewordsHT,
                                       textStrings, blStrings, 1, 0, bucketsize,
                                       elements, maxlength, maxcomplength);
}

size_t StringDictionaryHHTFC::getSize() {
    return bytesStrings * sizeof(uchar) + blStrings->getSize() +
           256 * 2 * sizeof(Codeword) + tableHT->getSize() +
           tableHU->getSize() + sizeof(StringDictionaryHHTFC);
}

void StringDictionaryHHTFC::save(ofstream &out) {
    ...
    saveValue<uint64_t>(out, buckets);
...
unique_ptr<StringDictionary> StringDictionaryHHTFC::load(ifstream &in) {
    size_t type = loadValue<uint32_t>(in);
    if (type != HHTFC) return NULL;

    unique_ptr<StringDictionaryHHTFC> dict =
        make_unique<StringDictionaryHHTFC>();

    dict->type = HHTFC;
    dict->elements = loadValue<uint64_t>(in);
    dict->maxlength = loadValue<uint32_t>(in);
    dict->maxcomplength = loadValue<uint32_t>(in);
    dict->buckets = loadValue<uint64_t>(in);
    ...
    dict->blStrings = make_shared<LogSequence>(in);
...
uchar *StringDictionaryHHTFC::getHeader(size_t idbucket) {
...
ChunkScan StringDictionaryHHTFC::decodeHeader(size_t idbucket) {
    uchar *ptr = textStrings + blStrings->getField(idbucket);
    ChunkScan chunk = {
        0, 0, ptr, maxcomplength, new uchar[4 * maxlength + tableHT->getK()],
        0, 0, 1};
...
bool StringDictionaryHHTFC::locateBucket(uchar *str, uint strLen,
                                         size_t *idbucket) {
...
void StringDictionaryHHTFC::locateBoundaryBuckets(uchar *str, uint strLen,
                                                  uint offset, size_t *left,
                                                  size_t *right) {
    size_t center = 0;
    int cmp = 0;
...
uint StringDictionaryHHTFC::searchPrefix(ChunkScan *c, uint scanneable,
                                         uchar *str, uint strLen) {
    uint_fast32_t id = NORESULT;
    ...
    while (true) {
        cmp = longestCommonPrefix(c->str + sharedCurr, str + sharedCurr,
                                  c->strLen - sharedCurr - 1, &sharedCurr);
...
uint StringDictionaryHHTFC::searchDistinctPrefix(ChunkScan *c, uint scanneable,
                                                 uchar *str, uint strLen) {
    uint id = 1;
...
void StringDictionaryHHTFC::resetScan(ChunkScan *c, size_t idbucket) {
    c->c_chunk = 0;
    c->c_valid = 0;
