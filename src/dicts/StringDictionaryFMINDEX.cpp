...
StringDictionaryFMINDEX::StringDictionaryFMINDEX(IteratorDictString *it,
    ...
    if (BWTsampling > 0) {
        ...
        for (uint i = 0; i < (len + 1 + W) / W; i++) bitmap[i] = 0;
        bitset(bitmap, 0);
    }
...
uint64_t StringDictionaryFMINDEX::locate(uchar *str, uint_fast32_t strLen) {
...
uchar *StringDictionaryFMINDEX::extract(uint64_t id, uint_fast32_t *strLen) {
    if ((id > 0) && (id <= elements)) {
        size_t i;
...
IteratorDictID *StringDictionaryFMINDEX::locatePrefix(uchar *str, uint strLen) {
    ...
    size_t left, right;
    uint num_occ =
        fm_index->locateP(prefix, (uint)(strLen + 1), &left, &right, elements);
    delete[] prefix;
...
IteratorDictID *StringDictionaryFMINDEX::locateSubstr(uchar *str, uint strLen) {
    if (BWTsampling == 0) {
        cout << "This dictionary configuration does not provide substring "
                "location"
             << endl;
        return NULL;
    }
...
uint StringDictionaryFMINDEX::locateRank(uint rank) { return rank; }

IteratorDictString *StringDictionaryFMINDEX::extractPrefix(uchar *str,
                                                           uint strLen) {
    uchar *prefix = new uchar[strLen + 2];
    prefix[0] = '\1';
    strncpy((char *)&(prefix[1]), (char *)str, strLen + 1);
    ...
    if (num_occ > 0)
        return new IteratorDictStringFMINDEX(fm_index, left, right - left + 1,
                                             elements, maxlength);
    else
        return NULL;
}

IteratorDictString *StringDictionaryFMINDEX::extractSubstr(
...
uchar *StringDictionaryFMINDEX::extractRank(uint64_t rank,
                                            uint_fast32_t *strLen) {
    return extract(rank, strLen);
}

IteratorDictString *StringDictionaryFMINDEX::extractTable() {
    return new IteratorDictStringFMINDEX(fm_index, 1, elements + 1, elements,
                                         maxlength);
}

size_t StringDictionaryFMINDEX::getSize() {
...
unique_ptr<StringDictionary> StringDictionaryFMINDEX::load(ifstream &in) {
    size_t type = loadValue<uint32_t>(in);
    if (type != FMINDEX) return NULL;

    auto dict = make_unique<StringDictionaryFMINDEX>();
...
void StringDictionaryFMINDEX::build_ssa(uchar *text, uint64_t len,
