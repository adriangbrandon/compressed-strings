...
HashBBdh::HashBBdh(uint _tsize) {
    ...
    for (i = 0; i < tsize; i++) hashtable[i] = (size_t)-1;
}
...
size_t HashBBdh::search(uchar *w, size_t len) {
    ...
    if (scmp(off_pos, w, len) == 0) return pos - 1;
...
size_t HashBBdh::getValue(size_t i) { return offsets->select1(i); }

size_t HashBBdh::getValuePos(size_t i) {
...
HashBBdh *HashBBdh::load(ifstream &fp) {
    ...
    offsets->setBit(h_new->hash->getField((h_new->b_ht)->select1(h_new->n)),
                    true);
