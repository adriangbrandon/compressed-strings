...
size_t Hashdh::search(uchar *w, size_t len) {
    size_t hval = bitwisehash(w, len, tsize);
    size_t next;

    if (!b_ht->access(hval)) return (size_t)-1;

    if (scmp(hash->getField(hval), w, len) == 0) return b_ht->rank1(hval) - 1;
    ...
    for (uint i = 1; i < tsize; i++) {
        ...
        if (!b_ht->access(next)) return (size_t)-1;
...
size_t Hashdh::getValue(size_t i) { return hash->getField(b_ht->select1(i)); }

size_t Hashdh::getValuePos(size_t i) { return hash->getField(i); }

size_t Hashdh::getSize() {
...
Hashdh *Hashdh::load(ifstream &fp) {
