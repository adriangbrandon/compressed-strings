...
/** Implementation of the XBW-Index proposed by Ferragina, Luccio, Manzini and
 * Muthukrishnan in FOCS 2005. The idea is patented, this code can't be
 * redistributed nor used for commercial purposes.
 *
...
class XBW {
   public:
    /** Builds an XBW object reading it from a file, it uses ssb to create the
    ...
    /** Searchs for path query in the tree, produces an array with the node ids
     ...
     */
    void subPathSearch(const uchar *qry, const uint ql, uint *left,
                       uint *right) const;
   ...
   protected:
    ...
    /** Searchs for path query in the tree. Computes the range of nodes that
     * satisfy the query up to the last item.
     * @param qry query, qry[0]=a and qry[1]=b means /a/b
     * @param ql length of the query
     * @param ini pointer for storing the initial position of the range
     * answering the query
     * @param fin pointer for storing the final position of the range answering
     * the query
     */
    /*
