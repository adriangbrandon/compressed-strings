...
XBW::XBW(ifstream &input) {
    ...
    // BitSequenceBuilder * sbb = new BitSequenceBuilderRG(2);		// 2, 4,
    // 20
...
void XBW::subPathSearch(const uchar *qry, const uint ql, uint *left,
                        uint *right) const {
    if (ql <= 1) {
        *left = 0;
        *right = nodesCount - 1;
