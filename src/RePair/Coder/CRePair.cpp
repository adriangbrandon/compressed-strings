...
void CRePair::compress(uint min_value, uint max_value, uint max_assigned,
                       uint m, uint n, uint *symbols, uint *symbols_pair) {
...
uint *CRePair::getSequence() { return symbols; }

Array *CRePair::getSymbolsGrammar() { return final_symbols_dict; }

BitSequence *CRePair::getBitsGrammar() { return BRR; }

bool CRePair::compress_pair_table() {
    uint aux, j, i;
    ...
    uint *symbols_pair_tmp =
        (uint *)malloc(sizeof(uint) * (max_assigned - max_value));
    ...
    symbols_new_bit =
        new uint[((symbols_new_len + (max_assigned - max_value)) / W + 1)];
    uint *symbols_new_value =
        (uint *)malloc(sizeof(uint) * (max_assigned - max_value));

    for (i = 0; i < ((symbols_new_len + (max_assigned - max_value)) / W + 1);
         i++)
        symbols_new_bit[i] = 0;
...

    {
        ...
        for (i = 0; i < m; i++) {
            if (symbols[i] > max_value) {
                cout << symbols[i] << " -> "
                     << symbols_new_value[symbols[i] - max_value - 1] +
                            max_value
                     << endl;
                symbols[i] =
...

    {
        ...
        BRR = new BitSequenceRG((uint *)symbols_new_bit,
                                (uint)symbols_new_bit_len, 20);
        delete[] symbols_new_bit;
        ...
        for (uint k = 0; k < symbols_new_len; k++)
            final_symbols_dict->setField(k, symbols_new[k]);
...
void CRePair::unroll(uint s, uint i) {
    if (s > max_value) {
...
void CRePair::new_value(uint *symbols_pair, uint *symbols_new_value, uint *k1,
                        uint *j, uint pos) {
    uint izq, der;
