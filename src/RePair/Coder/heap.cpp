...
void Heap::move(Tarray A, int64_t i, int64_t j, Trecord *rec) {
...
void Heap::incFreq(Theap *H, int64_t id) {
...
void Heap::insertHeap(Theap *H, int64_t id) {
    Trecord *rec = H->Rec->records;
    rec[id].hpos = ArrayG::insertArray(&H->infreq[1], id);
    rec[id].freq = 1;
}

int64_t Heap::extractMax(Theap *H) {
    //***    Trecord *rec = H->Rec->records;
    int64_t ret;
    Thnode *p;
    Thfreq *f;
...
void Heap::heapRepos(Theap *H, int64_t id) {
    Trecord *rec = H->Rec->records;
