...
#include "Adrian/irepair.h"
#include "Adrian/basics.h"
...
class RePairA {
   public:
    ...
    RePairA(int *sequence, uint64_t length, uchar maxchar);
    ...
    */
    static RePairA *load(std::ifstream &in);

    /** Loads a RePair encoding (without sequence) from an ifstream.
        @param in: the ifstream.
        @returns the loaded dictionary.
    */
    static RePairA *loadNoSeq(std::ifstream &in);
   ...
   protected:
    ...
    LogSequence
        *Cls;       //! RePair sequence (in a sequence of log bits per symbols)
