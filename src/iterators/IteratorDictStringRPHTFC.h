...
class IteratorDictStringRPHTFC : public IteratorDictString {
    ...
    */
    ...
    /** Generic destructor. */
    ~IteratorDictStringRPHTFC() { delete[] chunk.str; }
   ...
   protected:
    ...
    inline void decodeHeader() {
        ...
        chunk.b_ptr = ptr + blStrings->getField(bucket);
        bucket++;
        offset = 0;
    ...
    inline uint decodeSymbol() {
        ...
        while ((bitsrp - bitsproc) >= (8 - offset)) {
            symbol =
                (symbol << (8 - offset)) | (*chunk.b_ptr & mask(8 - offset));
        ...
        if (bitsrp > bitsproc) {
            ...
            symbol = (symbol << (bitsrp - bitsproc)) |
                     (*chunk.b_ptr >> (8 - offset));
        }
    ...
    inline void decodeNextString() {
        ...
        while (chunk.str[chunk.strLen - 1] != rp->maxchar) {
            ...
            if (rule >= rp->terminals)
                chunk.strLen += rp->expandRule(rule - rp->terminals,
                                               (chunk.str + chunk.strLen));
            else {
